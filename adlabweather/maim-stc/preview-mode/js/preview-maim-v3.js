/*
# ------------------------------------------------------------------------------
# | Preview-Mode for MA-IM: preview-maim-v2.js

# ------------------------------------------------------------------------------
# |
# | NOTE: v1 is not labeled as "preview-maim-v1.js". It's just labled "preview.js"
# |
# | Version ID: v2
# |
# | Updates that were made in this version from the previous v1:
# |
# ------------------------------------------------------------------------------
*/
...
labAd.preview.sizes = {
  layout:['sm','md', 'lg', 'xl'],
  overlay:['sm','md', 'lg', 'xl'], // xl and md added -  wodev comment
  raw:['sm','md', 'lg', 'xl']
};
...
function extraUIgone(){
  labAd.els.puifcoverlay = document.getElementById('previewUiFCOverlay');
  labAd.els.puifcoverlay.style.display = 'none';
...
labAd.preview.initForm = function() {
  ...
  try {
    ...
    labAd.els.previewFormClientName.innerHTML = labAd.els.previewFormClientName.innerHTML = labAd.meta.advertiser;
    ...
    if (labAd.deviceSize === 'lg') {
      ...
      document.querySelector('#deviceMedium').checked = true; // md condition added -  wodev comment
    }else if (labAd.deviceSize === 'xl') {
      document.querySelector('#deviceXLarge').checked = true; // xl condition added -  wodev comment
    ...
    // allow city/state to be edited
    var cityStateInput = document.getElementById('cityStateInput');

    cityStateInput.addEventListener('blur', function () { return cityStateInput.readOnly = true; });
    // select all when double click on input to change city and state
    cityStateInput.addEventListener('dblclick', function () {
        cityStateInput.readOnly = false;
        cityStateInput.select();
    });
    cityStateInput.addEventListener('keyup', function (evt) {
        if (evt.key === 'Enter') {
            cityStateInput.blur();
        }
    });
...
labAd.preview.imgParsing = function(){
  try {
    ...
    // modified this to match the file name need to be loaded - wodev comment
    labAd.preview.conditions = [];
    // All img.src values need to be adjusted for selected device size.
    labAd.preview.images.forEach(function(img){
      // determine the condition this img applies to by splitting the src and
      // using the 0 index of the result
      var sCond = '';
      var aCondParts = img.src.split('-');
      // if (aCondParts[0] !== 'all') {
      //   sCond = aCondParts[0];
      //   labAd.preview.conditions.push(sCond);
      //
      // } else {
      //
      // }
      // console.log(aCondParts[0])


      // iterate the array and prepend "labAd.deviceSize-" to each src value
      // before prepending the same value with the dir URL.
      // if (img.id !== 'logo') {
      //   img.src = labAd.preview.dirs[img.dir] + labAd.deviceSize + '-' + img.src;
      // } else {
      ...
      // console.log(aCondParts)
      if (aCondParts[0] === 'all') {
      // create object from array indices and id
        var iProps = {sz: aCondParts[0], type: aCondParts[1], condName:'', condValue:'', id: img.id};

      } else {
        var iProps = {sz: labAd.deviceSize, type: aCondParts[1], condName:'', condValue:'', id: img.id};


      }
        // console.log(iProps)

        // console.log(aSrc.join('-'))
    ...
    labAd.promises.allPreviewImages = Promise.all(labAd.promises.previewImages).then(function(aLoadedImages) {
      ...
      labAd.log({label:'labAd.promises.allPreviewImages fulfilled. ', detail: aLoadedImages, type: 'log'});
          ...
          aPreviewImages.forEach(function(img){
            ...
            }else if (img.id === 'uifc-'+ labAd.preview.adCond){
              labAd.els.previewUiFCOverlay.style.backgroundImage = 'url("'+ img.src +'")';
            }
          });
...
labAd.preview.modeInitOverlay = function() {
    ...
    labAd.els.previewUiFCOverlay = document.getElementById('previewUiFCOverlay');
